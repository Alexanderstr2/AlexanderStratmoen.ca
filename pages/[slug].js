...
function Post(props) {
  ...
  const title = `${props.title} // Alexander Stratmoen`
  const description = props.description || ''
  const url = `https://alexanderstratmoen.ca/${props.slug}`
  const date = new Date(props.date).toISOString()
  const image = props.image
    ? `https://alexanderstratmoen.ca${props.image}`
    : 'https://alexanderstratmoen.ca/static/images/home-opt.jpg'
  ...
  return (
    <>
      ...
      <ArticleJsonLd
        authorName="Alexander Stratmoen"
        type="Blog"
        url={url}
        title={title}
        images={[image]}
...
export async function getStaticProps({ params }) {
  try {
    ...
    const base = isProd ? 'https://alexanderstratmoen.ca' : 'http://localhost:3000'
